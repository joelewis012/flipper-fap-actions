...
self.addEventListener('push', e => {
  ...
  e.waitUntil(self.registration.showNotification(data.title || 'FlipperFAP', {
    ...
    icon: '/flipper-fap-actions/icon.png', badge: '/flipper-fap-actions/icon.png',
    tag: data.tag || 'fap-compile', data: data.url ? { url: data.url } : {},
...
self.addEventListener('notificationclick', e => {
  ...
  e.waitUntil(clients.matchAll({ type:'window' }).then(list => {
    for (const c of list) { if (c.url.includes('flipper-fap-actions') && 'focus' in c) return c.focus(); }
...
self.addEventListener('message', e => {
  if (e.data?.type === 'COMPILE_DONE') {
    ...
    self.registration.showNotification(
      ...
      { body: success ? `${short} compiled for ${firmware}. Tap to download.` : `${short} failed to compile.`,
        icon: '/flipper-fap-actions/icon.png', badge: '/flipper-fap-actions/icon.png', tag: 'fap-' + Date.now() }
    );
  }
});
